 ...
 * Copyr ight (c) Qualcomm Technologies, Inc. and/or its subsidiaries.
 */
...
#define QTI_8085_PHY_MODE_CFG_MASK			0x000f
#define QTI_8085_PHY_MODE_PSGMII_ALL_COPPER		0x0
#define QTI_8085_PHY_MODE_QSGMII_SGMII			0x4
#define QTI_8085_PQSGMII_ANALOG_SW_RESET		BIT(6)
#define QTI_8085_PQSGMII_MODE_CTRL_AZ_WORKAROUND	BIT(0)
#define QTI_8085_PQSGMII_TX_DRIVER_MASK			0x00f0
#define QTI_8085_PQSGMII_TX_DRIVER_SHIFT		4
#define QTI_8085_PQSGMII_TX_DRIVER_600MV		0xc
#define QTI_8085_SERDES_RESET_SLEEP_MS			100
...
enum qti_8075_phy_medium_t {
	QTI_8085_PHY_MEDIUM_COPPER = 0,
	QTI_8085_PHY_MEDIUM_FIBER = 1, /**< Fiber */
};

/* Phy pages */
enum qti_8075_phy_reg_pages_t {
	QTI_8085_PHY_SGBX_PAGES = 0, /* sgbx pages */
	QTI_8085_PHY_COPPER_PAGES = 1 /* copper pages */
};

enum qti_8075_package_mode {
	QTI_8075_PACKAGE_MODE_PSGMII,
	QTI_8075_PACKAGE_MODE_QSGMII,
};

struct qti_8075_device {
	struct mii_dev	*bus;
	u8 base_addr;
	enum qti_8075_package_mode package_mode;
	u8 tx_drive_strength;
};

static u32 qti_8075_package_configured;

static bool qti_8075_is_combo_addr(struct phy_device *phydev)
{
	return (phydev->addr & 0x7) == COMBO_PHY_ID;
}

static ofnode qti_8075_package_node(struct phy_device *phydev)
{
	ofnode phy_node, package_node;

	phy_node = phydev->node;
	if (!ofnode_valid(phy_node))
		return ofnode_null();

	package_node = ofnode_get_parent(phy_node);
...
static u8 qti_8075_package_base_addr(struct phy_device *phydev)
{
	ofnode package_node = qti_8075_package_node(phydev);
	u32 base_addr;

	if (ofnode_valid(package_node) &&
	    !ofnode_read_u32(package_node, "reg", &base_addr) &&
	    base_addr < PHY_MAX_ADDR)
		return base_addr;

	return phydev->addr & ~0x7;
}

static int qti_8075_package_mode_get(struct phy_device *phydev,
				     enum qti_8075_package_mode *mode)
{
	ofnode phy_node, package_node;
	const char *mode_str = NULL;

	*mode = QTI_8075_PACKAGE_MODE_PSGMII;

	package_node = qti_8075_package_node(phydev);
	if (ofnode_valid(package_node))
		mode_str = ofnode_read_string(package_node,
					      "qcom,package-mode");

	phy_node = phydev->node;
	if (!mode_str && ofnode_valid(phy_node))
		mode_str = ofnode_read_string(phy_node, "qcom,package-mode");

	if (!mode_str || !strcasecmp(mode_str, "psgmii"))
		return 0;

	if (!strcasecmp(mode_str, "qsgmii")) {
		*mode = QTI_8075_PACKAGE_MODE_QSGMII;
		return 0;
	}

	printf("QCA8075: unsupported package mode '%s'\n", mode_str);
	return -EINVAL;
}

static int qti_8075_tx_drive_strength_get(struct phy_device *phydev,
					  u8 *tx_drive_strength)
{
	ofnode package_node = qti_8075_package_node(phydev);
	u32 drive_strength = 600;

	if (ofnode_valid(package_node))
		ofnode_read_u32(package_node,
				"qcom,tx-drive-strength-milliwatt",
				&drive_strength);

	switch (drive_strength) {
	case 140:
		*tx_drive_strength = 0x0;
		return 0;
	case 160:
		*tx_drive_strength = 0x1;
		return 0;
	case 180:
		*tx_drive_strength = 0x2;
		return 0;
	case 200:
		*tx_drive_strength = 0x3;
		return 0;
	case 220:
		*tx_drive_strength = 0x4;
		return 0;
	case 240:
		*tx_drive_strength = 0x5;
		return 0;
	case 260:
		*tx_drive_strength = 0x6;
		return 0;
	case 280:
		*tx_drive_strength = 0x7;
		return 0;
	case 300:
		*tx_drive_strength = 0x8;
		return 0;
	case 320:
		*tx_drive_strength = 0x9;
		return 0;
	case 400:
		*tx_drive_strength = 0xa;
		return 0;
	case 500:
		*tx_drive_strength = 0xb;
		return 0;
	case 600:
		*tx_drive_strength = QTI_8085_PQSGMII_TX_DRIVER_600MV;
		return 0;
	default:
		printf("QCA8075: unsupported tx drive strength %u mV\n",
		       drive_strength);
		return -EINVAL;
	}
}

static int qti_8075_package_config(struct phy_device *phydev)
{
	struct qti_8075_device *dev = phydev->priv;
	enum qti_8075_package_mode package_mode;
	...
	ret = qti_8075_package_mode_get(phydev, &package_mode);
	if (ret)
		return ret;
	if (package_mode == QTI_8075_PACKAGE_MODE_QSGMII &&
	    !qti_8075_is_combo_addr(phydev)) {
		if (phydev->interface == PHY_INTERFACE_MODE_NA)
			phydev->interface = PHY_INTERFACE_MODE_QSGMII;

		if (phydev->interface != PHY_INTERFACE_MODE_QSGMII) {
			printf("QCA8075: qsgmii package requires qsgmii interface, got %s\n",
			       phy_string_for_interface(phydev->interface));
			return -EINVAL;
		}
	}

	base_addr = qti_8075_package_base_addr(phydev);
	ret = qti_8075_tx_drive_strength_get(phydev, &tx_drive_strength);
	if (ret)
		return ret;

	if (base_addr >= PHY_MAX_ADDR - PSGMII_ID)
		return -EINVAL;

	if (dev) {
		dev->base_addr = base_addr;
		dev->package_mode = package_mode;
		dev->tx_drive_strength = tx_drive_strength;
	}

	if (qti_8075_package_configured & BIT(base_addr))
		return 0;

	memcpy(&local_phydev, phydev, sizeof(struct phy_device));

	local_phydev.addr = base_addr + COMBO_PHY_ID;
	phy_data = phy_read(&local_phydev, MDIO_DEVAD_NONE,
			    QTI_8085_PHY_CHIP_CONFIG);
	if (phy_data < 0)
		return phy_data;

	phy_data &= ~QTI_8085_PHY_MODE_CFG_MASK;
	if (package_mode == QTI_8075_PACKAGE_MODE_QSGMII)
		phy_data |= QTI_8085_PHY_MODE_QSGMII_SGMII;
	else
		phy_data |= QTI_8085_PHY_MODE_PSGMII_ALL_COPPER;

	ret = phy_write(&local_phydev, MDIO_DEVAD_NONE,
			QTI_8085_PHY_CHIP_CONFIG, phy_data);
	if (ret < 0)
		return ret;

	local_phydev.addr = base_addr + PSGMII_ID;
	phy_data = phy_read(&local_phydev, MDIO_DEVAD_NONE,
			    QTI_8085_MODE_RESET_REG);
	if (phy_data < 0)
		return phy_data;

	phy_data &= ~QTI_8085_PQSGMII_ANALOG_SW_RESET;
	ret = phy_write(&local_phydev, MDIO_DEVAD_NONE,
			QTI_8085_MODE_RESET_REG, phy_data);
	if (ret < 0)
		return ret;

	mdelay(QTI_8085_SERDES_RESET_SLEEP_MS);
	...
	phy_data &= ~QTI_8085_PQSGMII_MODE_CTRL_AZ_WORKAROUND;
	ret = phy_write_mmd(&local_phydev, QTI_8085_PHY_MMD1_NUM,
			    QTI_8085_PSGMII_MODE_CTRL, phy_data);
	if (ret < 0)
		return ret;
	...
	phy_data &= ~QTI_8085_PQSGMII_TX_DRIVER_MASK;
	phy_data |= tx_drive_strength << QTI_8085_PQSGMII_TX_DRIVER_SHIFT;
	ret = phy_write(&local_phydev, MDIO_DEVAD_NONE,
			QTI_8085_PSGMII_TX_DRIVER_1_CTRL, phy_data);
	if (ret < 0)
		return ret;

	local_phydev.addr = base_addr + COMBO_PHY_ID;
	phy_data = phy_read_mmd(&local_phydev, QTI_8085_PHY_MMD3_NUM,
				QTI_8085_PHY_MMD3_ADDR_REMOTE_LOOPBACK_CTRL);
	if (phy_data < 0)
		return phy_data;

	phy_data &= ~BIT(1);
	ret = phy_write_mmd(&local_phydev, QTI_8085_PHY_MMD3_NUM,
			    QTI_8085_PHY_MMD3_ADDR_REMOTE_LOOPBACK_CTRL,
			    phy_data);
	if (ret < 0)
		return ret;

	qti_8075_package_configured |= BIT(base_addr);
	if (env_get_yesno("nss_debug_log") == 1)
		printf("QCA8075 package base=0x%x mode=%s txdrv=%u\n",
		       base_addr,
		       package_mode == QTI_8075_PACKAGE_MODE_QSGMII ?
		       "qsgmii" : "psgmii", tx_drive_strength);

	return 0;
}

static int qti_8075_qsdk_psgmii_config(struct phy_device *phydev)
{
	struct phy_device local_phydev;
	int phy_data;
	...
	phy_data &= ~BIT(1);
	phy_write_mmd(&local_phydev, QTI_8085_PHY_MMD3_NUM,
		      QTI_8085_PHY_MMD3_ADDR_REMOTE_LOOPBACK_CTRL,
		      phy_data);
	local_phydev.addr -= COMBO_PHY_ID;
 ...
 */
static enum qti_8075_phy_medium_t __phy_prefer_medium_get(struct phy_device *phydev)
{
 ...
 */
static enum qti_8075_phy_medium_t __phy_active_medium_get(struct phy_device *phydev)
{
	u16 phy_data = 0;
	...
	if (phy_mode == QTI_8085_PHY_PSGMII_AMDET) {
		phy_data = phy_read(phydev, MDIO_DEVAD_NONE, QTI_8085_PHY_SGMII_STATUS);
...
static int __phy_reg_pages_sel(struct phy_device *phydev,
			       enum qti_8075_phy_reg_pages_t phy_reg_pages)
{
	u16 reg_pages;
...
{
	enum qti_8075_phy_medium_t phy_medium;
	enum qti_8075_phy_reg_pages_t reg_pages;
...
static u8 __medium_is_fiber_100fx(struct phy_device *phydev)
{
	...
	if ((!(phy_data & QTI_8085_PHY4_AUTO_COPPER_SELECT)) &&
		...
		phy_data = phy_read(phydev, MDIO_DEVAD_NONE, QTI_8085_PHY_CHIP_CONFIG);
...
static u32 qti_8075_phy_restart_autoneg(struct phy_device *phydev)
{
	...
	if (qti_8075_is_combo_addr(phydev)) {
	...
	if (enable) {
		...
		if (!status) {
			...
			phy_write_mmd(phydev, QTI_8085_PHY_MMD3_NUM,
				      QTI_8085_PHY_MMD3_ADDR_8023AZ_TIMER_CTRL, phy_data);
		}
		phy_data = phy_read_mmd(phydev,	QTI_8085_PHY_MMD3_NUM,
					QTI_8085_PHY_MMD3_ADDR_CLD_CTRL5);
		phy_data &= ~(1 << 14);
		phy_write_mmd(phydev, QTI_8085_PHY_MMD3_NUM, QTI_8085_PHY_MMD3_ADDR_CLD_CTRL5,
			      phy_data);

		phy_data = phy_read_mmd(phydev, QTI_8085_PHY_MMD3_NUM,
					QTI_8085_PHY_MMD3_ADDR_CLD_CTRL3);
		phy_data &= ~(1 << 15);
		phy_write_mmd(phydev, QTI_8085_PHY_MMD3_NUM, QTI_8085_PHY_MMD3_ADDR_CLD_CTRL3,
			      phy_data);
...
static int qti_8075_phy_set_8023az(struct phy_device *phydev, u8 enable)
{
	...
	if (enable) {
		...
		phy_write_mmd(phydev, QTI_8085_PHY_MMD7_NUM,
			      QTI_8085_PHY_MMD7_ADDR_8023AZ_EEE_CTRL, phy_data);
		...
		phy_data &= ~0x0006;
		phy_write_mmd(phydev, QTI_8085_PHY_MMD7_NUM, QTI_8085_PHY_MMD7_ADDR_8023AZ_EEE_CTRL,
			      phy_data);
		if (phydev->drv->uid == QTI_8085_PHY_V1_0_5P) {
			phy_write_mmd(phydev, QTI_8085_PHY_MMD3_NUM,
				      QTI_8085_PHY_MMD3_ADDR_8023AZ_TIMER_CTRL,
...
static int qti_8075_probe(struct phy_device *phydev)
{
	...
	phydev->priv = dev;
	dev->bus = phydev->bus;
	dev->base_addr = qti_8075_package_base_addr(phydev);
	dev->package_mode = QTI_8075_PACKAGE_MODE_PSGMII;
	dev->tx_drive_strength = QTI_8085_PQSGMII_TX_DRIVER_600MV;
...
static int qti_8075_config(struct phy_device *phydev)
{
	...
	ret = qti_8075_package_config(phydev);
	if (ret < 0)
		return ret;

	base_addr = qti_8075_package_base_addr(phydev);

	phy_data = phy_read_mmd(phydev, QTI_8085_PHY_MMD7_NUM, QTI_8085_PHY_MMD7_DAC_CTRL);
	if (phy_data < 0)
		return phy_data;
	...
	if (phydev->drv->uid == QTI_8085_PHY_V1_0_5P) {
		...
		phy_data &= 0xbfff;
		phy_write_mmd(&local_phydev, QTI_8085_PHY_MMD1_NUM, QTI_8085_PSGMII_FIFI_CTRL,
			      phy_data);
	}
	...
	if (phydev->drv->uid == QTI_8085_PHY_V1_0_5P ||
		...
		for (port_id = 0; port_id < 5; port_id++) {
			 ...
			 */
			phy_data = phy_read_mmd(&local_phydev, QTI_8085_PHY_MMD7_NUM,
			...
			phy_data |= QTI_8085_LED_1000_CTRL1_100_10_MASK;
			phy_write_mmd(&local_phydev, QTI_8085_PHY_MMD7_NUM,
				      QTI_8085_PHY_MMD7_LED_1000_CTRL1, phy_data);
		}
	}

	if (!dev || dev->package_mode == QTI_8075_PACKAGE_MODE_PSGMII)
		return qti_8075_qsdk_psgmii_config(phydev);
...
U_BOOT_PHY_DRIVER(qti_8075_v1_0_5P_driver) = {
...
U_BOOT_PHY_DRIVER(qti_8075_v1_1_5P_driver) = {
...
U_BOOT_PHY_DRIVER(qti_8075_v1_1_2P_driver) = {
