...
static void qti_8x8x_split_mii_addr(u32 regaddr, u16 *r1,
				    u16 *r2, u16 *page, u16 *switch_phy_id)
{
	*r1 = regaddr & 0x1c;
...
static u32 qti_8x8x_mii_read(struct phy_device *phydev, u32 reg)
{
	u16 r1, r2, page, switch_phy_id;
	u16 lo, hi;
	struct phy_device local_phydev;

	qti_8x8x_split_mii_addr((u32)reg, &r1, &r2,
				&page, &switch_phy_id);
...
static void qti_8x8x_mii_write(struct phy_device *phydev, u32 reg,
			       u32 val)
{
	u16 r1, r2, page, switch_phy_id;
	u16 lo, hi;
...
static void qti_8x8x_mii_update(struct phy_device *phydev, u32 reg,
				u32 mask, u32 val)
{
	u32 new_val = 0, org_val = 0;
...
{
//	u32 reg_id_c45 = QTI_8X8X_REG_C45_ADDRESS(mmd_num, reg_id);
	struct phy_device local_phydev;
...
static void qti_8x8x_phy_modify_mmd(struct phy_device *phydev,
				    u32 phy_addr, u32 mmd_num,
		u32 mmd_reg, u32 mask, u32 value)
{
...
static void qti_8x8x_core_clk_enable(struct phy_device *phydev, u32 reg)
{
	u32 val;
...
static void qti_8x8x_core_clk_disable(struct phy_device *phydev, u32 reg)
{
	u32 val;
...
static void qti_8x8x_core_clk_reset(struct phy_device *phydev, u32 reg)
{
	u32 val;
...
static u64 qti_8x8x_uniphy_raw_clock[QTI_8X8X_UNIPHY_INSTANCE * 2] = {0};
...
static void qti_8x8x_clk_update(struct phy_device *phydev, u32 cmd_reg)
{
	u32 i, reg_val;
	...
	pr_dbg("CLK cmd reg 0x%x fails updating to new configurations\n",
	       cmd_reg);
}
...
static u8 qti_8x8x_clk_is_enabled(struct phy_device *phydev,
				  const char *clock_id)
{
	struct clk_lookup *clk;
	u32 cbc_reg = 0, reg_val = 0;
...
static void qti_8x8x_clk_parent_set(struct phy_device *phydev,
				    const char *clock_id, enum qti_8x8x_clk_parent_t parent)
{
	struct clk_lookup *clk;
	u32 i, reg_val;
...
static void qti_8x8x_uniphy_raw_clock_set(enum qti_8x8x_clk_parent_t uniphy_clk,
					  u64 rate)
{
	switch (uniphy_clk) {
...
static u64 qti_8x8x_uniphy_raw_clock_get(enum qti_8x8x_clk_parent_t
		uniphy_clk)
{
	switch (uniphy_clk) {
	case QTI_8X8X_P_UNIPHY0_RX:
				  ...
				  const char *clock_id, u32 rate)
{
	...
	if (rate == UQXGMII_XPCS_SPEED_2500M_CLK) {
		if (prate != UQXGMII_SPEED_2500M_CLK) {
			pr_dbg("CLK %s parent(%lld) needs to be updated to %d\n",
			       clock_id,
	...
	if (cdiv_reg == 0 && cdiv_val > 0) {
		pr_dbg("CLK %s needs CDIVR to generate rate %d from prate %lld\n",
		       clock_id, rate, prate);
		return;
	}
...
#if 0
void qti_8x8x_port5_uniphy0_clk_src_get(u8 *bypass_en)
{
	u32 reg_val = 0;
...
U_BOOT_CMD(qti_8x8x_clk_dump, 1, 1, do_qti_8x8x_clk_dump,
...
static void qti_8x8x_clk_ids_get(u32 qti_8x8x_port_id,
...
static void qti_8x8x_port_clk_en_set(struct phy_device *phydev,
				     u32 qti_8x8x_port_id, u8 mask, u8 enable)
{
	char *clk_ids[QTI_8X8X_PORT_CLK_CBC_MAX + 1] = {NULL};
	u32 i = 0;
...
static void qti_8x8x_gcc_common_clk_parent_enable(struct phy_device *phydev,
						  enum qti_8x8x_work_mode_t clk_mode)
{
	/* Switch core */
	...
	if (clk_mode == QTI_8X8X_PHY_UQXGMII_MODE ||
	    clk_mode == QTI_8X8X_PHY_SGMII_UQXGMII_MODE)
...
static void qti_8x8x_gcc_port_clk_parent_set(struct phy_device *phydev,
					     enum qti_8x8x_work_mode_t clk_mode,
					     u32 qti_8x8x_port_id)
{
	enum qti_8x8x_clk_parent_t port_tx_parent, port_rx_parent;
...
static int qti_8x8x_gcc_clock_init(struct phy_device *phydev,
				   enum qti_8x8x_work_mode_t clk_mode, u32 pbmp)
{
	u32 qti_8x8x_port_id = 0;
	/* clock type mask value for 6 manhattan ports */
	u8 clk_mask[DEV_8X8X_PORT_5 + 1] = {0};
	u8 switch_flag = 0;
	enum qti_8x8x_clk_parent_t uniphy_index = QTI_8X8X_P_UNIPHY0_RX;
	...
	while (uniphy_index <= QTI_8X8X_P_UNIPHY1_TX) {
		...
		if (qti_8x8x_uniphy_raw_clock_get(uniphy_index) == 0)
	...
	pr_dbg("QTI_8X8X GCC CLK initialization with clock mode %d on port bmp 0x%x\n",
	       clk_mode, pbmp);
	return 0;
}
					...
					u32 uniphy_addr)
{
	...
	while (calibration_done != QTI_8X8X_UNIPHY_MMD1_CALIBRATION_DONE) {
		mdelay(1);
		if (retries-- == 0)
			pr_dbg("uniphy calibration time out!\n");
...
static void qti_8x8x_port_speed_clock_set(struct phy_device *phydev,
					  u32 qti_8x8x_port_id, u32 speed)
{
	u32 clk_rate = 0;
...
static void qti_8x8x_ephy_addr_get(struct phy_device *phydev,
				   u32 qti_8x8x_port_id, u32 *phy_addr)
{
	u32 data = 0;
...
static u16 qti_8x8x_uniphy_xpcs_mmd_read(struct phy_device *phydev,
					 u16 mmd_num, u16 mmd_reg)
{
	u32 uniphy_xpcs_addr = 0;
...
static void qti_8x8x_uniphy_xpcs_mmd_write(struct phy_device *phydev,
					   u16 mmd_num, u16 mmd_reg, u16 reg_val)
{
	u32 uniphy_xpcs_addr = 0;
#ifdef DEBUG
	u16 phy_data = 0;
...
static u32 qti_8x8x_uniphy_xpcs_port_to_mmd(u32 qti_8x8x_port_id)
{
	u32 mmd_id = 0;
...
static void qti_8x8x_uniphy_xpcs_modify_port_mmd(struct phy_device *phydev,
						 u32 qti_8x8x_port_id, u32 mmd_reg,
		u32 mask, u32 value)
{
	u32 mmd_id = 0;
...
static void qti_8x8x_uniphy_xpcs_autoneg_restart(struct phy_device *phydev,
						 u32 qti_8x8x_port_id)
{
	u32 retries = 500, uniphy_data = 0, mmd_id = 0;
...
static void _qti_8x8x_interface_uqxgmii_mode_set(struct phy_device *phydev,
						 u32 uniphy_addr)
{
	u32 qti_8x8x_port_id = 0, phy_addr = 0;
...
{
	u32 uniphy_addr = 0, qti_8x8x_port_id = 0;
	...
	pr_debug("Mii using mii-4bits, configure as PHY mode, ");
	pr_debug("enable autoneg ability, disable TICD\n");
...
static void qti_8x8x_interface_sgmii_mode_set(struct phy_device *phydev,
					      u32 uniphy_index, u32 qti_8x8x_port_id,
					      struct mac_config_t *config)
{
	...
	if (uniphy_index == QTI_8X8X_UNIPHY_SGMII_1) {
		pr_dbg("uniphy1 xpcs reset, confiugre raw clock as:%lld\n",
		       raw_clk);
		qti_8x8x_clk_assert(phydev, QTI_8X8X_UNIPHY_XPCS_RST);
		qti_8x8x_uniphy_raw_clock_set(QTI_8X8X_P_UNIPHY1_RX, raw_clk);
		qti_8x8x_uniphy_raw_clock_set(QTI_8X8X_P_UNIPHY1_TX, raw_clk);
	} else {
		pr_dbg("uniphy0 configure raw clock as %lld\n", raw_clk);
		qti_8x8x_uniphy_raw_clock_set(QTI_8X8X_P_UNIPHY0_RX, raw_clk);
		qti_8x8x_uniphy_raw_clock_set(QTI_8X8X_P_UNIPHY0_TX, raw_clk);
	}
...
static int8_t qti_8x8x_uniphy_mode_check(struct phy_device *phydev, u32 uniphy_index,
					 enum qti_8x8x_uniphy_mode_t uniphy_mode)
{
	u32 uniphy_addr = 0;
	u16 uniphy_mode_ctrl_data = 0;
...
static enum qti_8x8x_pin_config_param pinconf_to_config_param(unsigned long config)
{
...
static void qti_8x8x_work_mode_set(struct phy_device *phydev,
				   enum qti_8x8x_work_mode_t work_mode)
{
	u32 data = 0;
				   ...
				   phy_interface_t mode0, phy_interface_t mode1)
{
	...
	switch (mode0) {
	//:TODO  /*PHY_INTERFACE_MODE_2500BASEX need to replace*/
...
{
	if (cpu_bmp & BIT(port_id) || port_id == DEV_8X8X_PORT_0 || port_id == DEV_8X8X_PORT_5)
		return false;
					  ...
					  u32 port_id, bool enable, u32 cpu_bmp)
{
	...
	if (!qti_8x8x_port_phy_connected(phydev, port_id, cpu_bmp)) {
...
static void port_rxhdr_mode_set(struct phy_device *phydev,
				u32 port_id, enum port_header_mode_t mode)
{
	u32 val = 0;

	if (mode == FAL_NO_HEADER_EN) {
		val = 0;
	} else if (mode == FAL_ONLY_MANAGE_FRAME_EN) {
		val = 1;
	} else if (mode == FAL_ALL_TYPE_FRAME_EN) {
		val = 2;
	} else {
		printf("%s %d Error: Bad param\n", __func__, __LINE__);
		return;
				    ...
				    u32 port_id, u32 duplex, u32 cpu_bmp)
{
	...
	if (!qti_8x8x_port_phy_connected(phydev, port_id, cpu_bmp)) {
		...
		printf("%s %d Error: Duplex/Speed set for QTI_8X8X ", __func__, __LINE__);
		printf("DEV_8X8X_PORT_1-4 is not implemented\n");
		return -EINVAL;
	}
	return 0;
}
				   ...
				   u32 port_id, u32 speed, u32 cpu_bmp)
{
	...
	if (!qti_8x8x_port_phy_connected(phydev, port_id, cpu_bmp)) {
		...
		printf("%s %d Error: Duplex/Speed set for ", __func__, __LINE__);
		printf("QTI_8X8X DEV_8X8X_PORT_1-4 is not implemented\n");
		return -EINVAL;
	}
		...
		struct qti_8x8x_port_info *port_info)
{
	...
	enum qti_8x8x_work_mode_t work_mode = QTI_8X8X_SWITCH_MODE;
	...
	if (mac_mode == PHY_INTERFACE_MODE_2500BASEX) {
				   ...
				   u32 port_id, u32 *number)
{
	...
	if (*number > ISISC_QOS_PORT_TX_BUFFER_MAX) {
		printf("%s %d Bad param\n", __func__, __LINE__);
		return;
	}
				   ...
				   u32 port_id, u32 *number)
{
	...
	if (*number > ISISC_QOS_PORT_RX_BUFFER_MAX) {
		printf("%s %d Bad param\n", __func__, __LINE__);
		return;
	}
...
{
	u32 phy_addr = phydev->addr, qti_8x8x_port_id = phydev->addr;
	u32 status = phydev->link, new_speed = phydev->speed;
	u32 port_clock_en = 0;
	...
	pr_dbg("UNIPHY GMII/XGMII interface and ETHPHY GMII interface reset and release\n");
	qti_8x8x_port_clk_reset(phydev, qti_8x8x_port_id,
				QTI_8X8X_CLK_TYPE_UNIPHY | QTI_8X8X_CLK_TYPE_EPHY);

	pr_dbg("ipg_tune and xgmii2gmii reset for uniphy and ETHPHY, function reset\n");
...
static int qti_8x8x_config(struct phy_device *phydev)
{
	...
	if (dev->mode != DEV_8X8X_SWITCH_MODE &&
		...
		if (dev->mode == DEV_8X8X_BYPASS_MODE) {
			...
			ret = qti_8x8x_bypass_interface_mode_set(&local_phydev);
			if (ret)
				return ret;
		}
		...
		for (i = 0; i < DEV_8X8X_MAX_PORTS; i++) {
			...
			switch_cfg->port[i].mode = ofnode_read_phy_mode(port_node);
			switch_cfg->port[i].addr = ofnode_read_u32_default(port_node,
									   "phy_addr", -1);
			switch_cfg->port[i].label = ofnode_read_string(port_node, "label");
...
U_BOOT_PHY_DRIVER(qti_8x8x_driver) = {
